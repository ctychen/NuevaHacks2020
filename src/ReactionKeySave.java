...
public class ReactionKeySave {
	...
	String[] congrats = {"avoided touching your face", "dodged a sneeze", "dodged a cough"};
	String[] reee = {"touched your face", "got hit by a sneeze", "got coughed on"};
	int key;
	int message;
	int maxTime;
	int time;
	...
	public void draw(PApplet g, boolean keys[]) {
		...
		if ((keys[keyCodes[key]] || saved == 1) && saved != -1) { //saved
			g.text("NICE SAVE, you " + congrats[message], g.width*0.6f, g.height*0.45f);
			saved = 1;
			
		} else if ((time > maxTime || saved == -1) && saved != 1) {
			g.text("TOO SLOW! You " + reee[message], g.width*0.5f, g.height*0.5f);
